...
if [ "$FORCE_RUN" = "true" ]; then
    MAKE_EXTRA_ARGS="KEEP_DISPLACEMENT=1"
fi
...
if [ -n "$TARGET" ]; then
    OUTPUT_SUBDIR="output_$TARGET"
fi

# MakefileのINPUT_NAMEに対応した出力先を使う
# (IGA, SSIGA等すべてのターゲットに適用)
if [ -f "$SCRIPT_DIR/Makefile" ]; then
    INPUT_NAME=$(awk -F':=' '/^[[:space:]]*INPUT_NAME[[:space:]]*:=[[:space:]]*/ {gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2); print $2; exit}' "$SCRIPT_DIR/Makefile")
    if [ -n "$INPUT_NAME" ]; then
        OUTPUT_SUBDIR="output_$INPUT_NAME"
    fi
fi
...
if [ -n "$REMOTE_PID" ]; then
    ...
    if [ "$FORCE_RUN" = "true" ]; then
        ...
        if [ "$UNSYNCED_COUNT" -gt 0 ]; then
            ...
            mkdir -p "$SCRIPT_DIR/analysis/$OUTPUT_SUBDIR"
            rsync -azP "$HOST:$REMOTE_DIR/analysis/$OUTPUT_SUBDIR/" "$SCRIPT_DIR/analysis/$OUTPUT_SUBDIR/" 2>/dev/null || true
            rsync -azP "$HOST:$REMOTE_DIR/remote_log.txt" "$SCRIPT_DIR/analysis/$OUTPUT_SUBDIR/" 2>/dev/null || true
...
if [ -n "$TARGET" ] && [ "$MODE" != "RERUN" ]; then
    ...
    if ! ssh "$HOST" "$REMOTE_ENV cd $REMOTE_DIR && make -n $MAKE_EXTRA_ARGS $TARGET >/dev/null 2>&1"; then
...
if [ "$MODE" = "RERUN" ]; then
    ...
    MAKE_CMD="bash -lc 'cd $REMOTE_DIR && $REMOTE_ENV TERM=xterm-256color CLICOLOR_FORCE=1 FORCE_COLOR=1 stdbuf -oL -eL make $MAKE_EXTRA_ARGS $TARGET 2>&1 | tee -a $REMOTE_LOG_COLOR >(stdbuf -oL sed -r '\''s/\x1B\[[0-9;]*[A-Za-z]//g'\'' >> $REMOTE_LOG)'"
...
rsync -azP "$HOST:$REMOTE_DIR/analysis/$OUTPUT_SUBDIR/" "$SCRIPT_DIR/analysis/$OUTPUT_SUBDIR/" 2>/dev/null || true
rsync -azP "$HOST:$REMOTE_DIR/remote_log.txt" "$SCRIPT_DIR/analysis/$OUTPUT_SUBDIR/" 2>/dev/null || true
rsync -azP "$HOST:$REMOTE_DIR/remote_log_color.txt" "$SCRIPT_DIR/analysis/$OUTPUT_SUBDIR/" 2>/dev/null || true
